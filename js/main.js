...
function showSlides(n) {
  var slides = document.getElementsByClassName("mySlides"); //takes all elements with the class name "myslides" and stores them in variable array "slides"
  ...
  for (i = 0; i < dots.length; i++) {
    dots[i].className = dots[i].className.replace("active", ""); //for loop takes each item in the array "dots" and removes "active" which removes the acive styling
  }
  slides[slideIndex - 1].style.display = "block"; //displays the image in the slideshow
  dots[slideIndex - 1].className += "active"; //adds the active styling the the dot associated with the image
}

//code will create close the contact from when th user clicks off of it
//first step is to add an event listener for any clicks on the webstie
